...
router.get('/', (req, res) => {
  // Most recently edited first -- updated_at is kept current by the touch
  // triggers in db.js for any change to the project or anything in it, not
  // just renames, so this doubles as "most recently active project first".
...
router.post('/', (req, res) => {
  ...
  const result = db
    .prepare("INSERT INTO projects (name, description, updated_at) VALUES (?, ?, datetime('now'))")
