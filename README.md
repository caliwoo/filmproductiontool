...
- **Locations** — location directory (name, address, notes) used by scenes and shoot days; edit any field inline, no separate save step. A **Scenes** column lists which scene numbers use each location, in script order, so you can see everywhere a location is needed at a glance. Importing a script auto-populates this list: each scene's heading (e.g. "APARTMENT - BEDROOM") is grouped by its first segment ("APARTMENT") into a read-only **Scene Heading** column, so sub-rooms in the same building share one location — that's the set as the script called it, not a real place, so the **Name** column right before it starts blank for you to fill in with the location's actual real name/address once you've scouted or booked it; anywhere a location's name would show before that (a scene's location picker, the call sheet, PDF breakdown sheet, schedule) falls back to showing the scene heading instead of a blank. Scenes imported before this existed are backfilled automatically the next time the server starts, and any location an earlier version of this feature already auto-named is migrated into the new Scene Heading column too, without touching one you've since renamed yourself.
